<?php
namespace Matters\GeneticSim\Test\Containers;
use Exception;
use GeneticCachedDIContainer;
...
class ContainerTest extends TestCase {
    ...
    /** @var GeneticCachedDIContainer $subject */
    private $subject;
    ...
    public function testDefinedServicesWithErrors() {
        $oldErrorHandler = set_error_handler('Matters\GeneticSim\Test\Containers\ContainerTestErrorHandler');
        $failure = [];
        foreach ($this->subject->getServiceIds() as $sId) {
            set_error_handler('Matters\GeneticSim\Test\Containers\ContainerTestErrorHandler'); //yes, some services override this
            try {
                $this->assertNotNull($this->subject->get($sId));
